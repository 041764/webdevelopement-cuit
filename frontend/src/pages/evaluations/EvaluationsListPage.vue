...
import { NButton, NCard, NDataTable, NInput, NPagination, NSpace, useMessage } from 'naive-ui'
...
import type { Evaluation } from '@/api/schema'
    ...
    render: (row) =>
      h(
        NButton,
        {
          ...
          secondary: true,
          onClick: () => {
            void router.push({ name: 'evaluations-detail', params: { evaluationId: row.id } })
          },
        },
        { default: () => '详情' },
      ),
...
async function goCreate() {
  await router.push({ name: 'evaluations-new' })
}
